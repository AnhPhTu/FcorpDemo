import { AlertCommon, Buttons, CommentsArea, Header, ProductItem, SafeAreaStatusBar, SafeAreaViewCus, TextCus, TextInputCus, TouchCus } from 'components';
...
const ProductDetails = () => {
    ...
    const [dummyDataComment, setDummyDataComment] = useState<any>( {
      ...
      children: [
        ...
        }, 
        {
          ...
          commentator :' Jack',
          responseTo :'',
          comments: 'Tôi muốn đặt hàng sản phẩm này với số lượng 70 cái liệu có được không??',
          commentdate: '18:30 11/10/2023',
          children: [
            {
              commentTreeLevel : 2,
              ...
              commentator :' NV2',
              responseTo :'Jack',
              comments: 'sản phẩm này với số lượng 70 sẽ có giá là 7000000, bạn muốn thanh toán qua đơn vị nào, chuyển khoản hay tiền mặt ạ?',
              commentdate: '18:30 11/10/2023',
              children: []
            },
            {
    ...
    const onAddComment =(item) => {
      ...
      if(item?.commentTreeLevel > 0) {
        const belongTo = itemHandleAction.belongTo?.length > 1 ? itemHandleAction.belongTo : [0,itemHandleAction.id]
        dummyAddItem.commentTreeLevel = 2;
        dummyAddItem.belongTo = belongTo ??  [0];
        dummyAddItem.comments = item.comments;
        dummyAddItem.id = Date.now()
        const index = commentData.children.map(object => object.id).indexOf(belongTo[1]);
    ...
    const onConfirmEdit = (comment: string) => {
      ...
      if(itemHandleAction.commentTreeLevel === 1) {
        const index = commentData.children.map(object => object.id).indexOf(itemHandleAction.id);
        commentData.children[index].comments = comment;
        
      }
      if(itemHandleAction.commentTreeLevel === 2) {
        const indexRoot = commentData.children.map(object => object.id).indexOf(itemHandleAction.belongTo[1]);
        const indexLeaf1 = commentData.children[indexRoot].children.map(object => object.id).indexOf(itemHandleAction.id);
    ...
    const handleAction = useCallback((action) => {
      ...
      const deleteItem = () => {
          if(itemHandleAction.commentTreeLevel === 1) {
            ...
            commentData.children = commentData.children.filter(_item => _item.id !== itemHandleAction.id); 
          }
          if(itemHandleAction.commentTreeLevel === 2) {
            const index = commentData.children.map(object => object.id).indexOf(itemHandleAction.belongTo[1]);
            commentData.children[index].children = commentData.children[index].children.filter(_item => _item.id !== itemHandleAction.id); 
          }
          setDummyDataComment(commentData);
      }
      const handleDelete =  () => {
    ...
    return (
        ...
        <SafeAreaViewCus style={[globalStyles.bgWhite, globalStyles.flex1]}>
          ...
          <TextCus
                  ...
                  ]}>{params?.product ? params.product : 'Product Details'}</TextCus>
            <View style = {[{paddingHorizontal: 2,backgroundColor: '#c2c0c0', marginBottom: 12}]}>
            ...
            <CommentsArea commentData = {dummyDataComment} 
          ...
          {!isEdit ? (<View style={[{paddingVertical: 10, paddingLeft: 10, backgroundColor : Colors.color09}]}>
          {isResponse ? (<View style = {[globalStyles.rowItem, {paddingBottom:5}]}>
          ...
          <TouchCus style ={[{paddingLeft:5}]} onPress={() => { setIsResponse(false) }}>
                    ...
                    <Controller
                      ...
                      render={() => (
                        <View style={[styles.rowItem, styles.textArea]}>
                          <TextInputCus
                            style={[styles.flex1, styles.mr8, {height:30, opacity : 1}]}
                            autoCapitalize="none"
        ...
        <View
          ...
          {baseAction.map((title, index: number) => {
            if(itemHandleAction.commentator !== 'User' && (title === 'Delete' || title === 'Edit')) {
              return null
            }
            return (
              <View key={index}>
